...
if (config.server.update) {
    requester(config.server.link, (err, req, data) => {
        ...
        if (config.server.version < requesterConfig.server.version) {
            ...
            logger.warn(`[SERVER] Descargar -> https://github.com/perrop/Alexis-New-Actualizacion`)
            process.exit()
        } else {
...
New Version Works
Suscribe Bots Free: https://www.youtube.com/channel/UCh5X1IjfvpbL7NBbTkoD58gview_as=subscriber
`);
const game = {
    url: '',
    protocolVersion: 0,
                ...
                if (user.startedBots && !stoppingBots) {
                    ...
                    setInterval(() => {
                        if (seconds === 5) {
                            ...
                            logger.warn(`[SERVER] stop bots en ${5 - seconds} seconds`);
                            seconds++
                        }
                    }, 1000)
                }
